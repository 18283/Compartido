...
class Simplex(object):
    def __init__(self, num_vars, constraints, objective_function):
        ...
        self.objective = objective_function[0] #max/min
    ...
    #construye matriz de las restricciones
    def construct_matrix_from_constraints(self):
        num_s_vars = 0  # variables de holguras o superfluas
        num_r_vars = 0  # variables artificiales
        ...
        coeff_matrix = [[Fraction("0/1") for i in range(total_vars+1)] for j in range(len(self.constraints)+1)] ##matriz
        ...
        for i in range(1, len(self.constraints)+1):
            ...
            for j in range(len(constraint)):
                ...
                if '_' in constraint[j]:
                    ...
                    if constraint[j-1] is '-':
                        ##(modificado)coeff_matrix[i][int(index)-1] = Fraction("-" + coeff[:-1] + "/1")
                        if '/' in coeff[:-1]:  # Verifica si el coeficiente es fraccionario
                            coeff_matrix[i][int(index) - 1] = Fraction("-" + coeff[:-1])  # Agrega el signo negativo a la fracción directamente
                        else:
                            coeff_matrix[i][int(index) - 1] = Fraction("-" + coeff[:-1] + "/1")  # Si es entero, añade "/1" después del signo negativo

                    else:
                        ##(modificado)coeff_matrix[i][int(index)-1] = Fraction(coeff[:-1] + "/1")
                        if '/' in coeff[:-1]:  # Verifica si el coeficiente es fraccionario
                            coeff_matrix[i][int(index) - 1] = Fraction(coeff[:-1])  # Convierte el coeficiente en una fracción directamente
                        else:
                            coeff_matrix[i][int(index) - 1] = Fraction(coeff[:-1] + "/1")  # Si es entero, añade "/1"


                elif constraint[j] == '<=':
                    coeff_matrix[i][s_index] = Fraction("1/1")  # anade variable de holgura
                    s_index += 1

                elif constraint[j] == '>=':
                    coeff_matrix[i][s_index] = Fraction("-1/1")  # anade variable superflua
                    coeff_matrix[i][r_index] = Fraction("1/1")   # anade variable artificial
                    s_index += 1
                    r_index += 1
                    r_rows.append(i)
            ...
            #(modificado)coeff_matrix[i][-1] = Fraction(constraint[-1] + "/1")
            last_term = constraint[-1]

            # Verificamos si el término independiente ya es una fracción
            if '/' in last_term:
                coeff_matrix[i][-1] = Fraction(last_term)  # Se convierte directamente a Fraction
            else:
                coeff_matrix[i][-1] = Fraction(last_term + "/1")  # Se convierte en fracción dividiendo entre 1
        ...
        for i in range(len(objective_function_coeffs)):
            if '_' in objective_function_coeffs[i]:
                ...
                if objective_function_coeffs[i-1] is '-':
                    #(modificado)self.coeff_matrix[0][int(index)-1] = Fraction(coeff[:-1] + "/1")
                    if '/' in coeff[:-1]:
                    ...
                    #(modificado)self.coeff_matrix[0][int(index)-1] = Fraction("-" + coeff[:-1] + "/1")
                    if '/' in coeff[:-1]:
                        # Si ya es una fracción, simplemente prepende el signo negativo y convierte a Fraction
                        self.coeff_matrix[0][int(index)-1] = Fraction("-" + coeff[:-1])
                    else:
                        # Si es un entero, añade "/1" al final para crear una fracción con denominador 1
                        self.coeff_matrix[0][int(index)-1] = Fraction("-" + coeff[:-1] + "/1")
